...
function CarDetailInner() {
  ...
  if (!car) return <div className="p-6 text-muted-foreground">Car not found</div>;
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
      <Link to="/cars" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-2">
      ...
      <Card className="luxury-card p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{[car.year, car.make, car.model].filter(Boolean).join(" ") || "—"}</p>
            {car.license_plate && <p className="text-xs text-muted-foreground/70 mt-1">Plate: {car.license_plate}</p>}
          </div>
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <Input value={mileageInput} onChange={(e) => setMileageInput(e.target.value)} className="input-luxury w-28" type="number" />
              <span className="text-xs text-muted-foreground/70">mi</span>
            </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card className="luxury-card p-3 text-center">
          ...
          <div className="text-lg font-bold text-foreground">${totalMaintCost.toFixed(0)}</div>
          <div className="text-[10px] text-muted-foreground/70">Maintenance ({maintenance.length})</div>
        </Card>
        <Card className="luxury-card p-3 text-center">
          <Fuel className="w-5 h-5 mx-auto text-blue-400 mb-1" />
          <div className="text-lg font-bold text-foreground">${totalFuelCost.toFixed(0)}</div>
          <div className="text-[10px] text-muted-foreground/70">Fuel ({fuel.length})</div>
        </Card>
        <Card className="luxury-card p-3 text-center">
          <CreditCard className="w-5 h-5 mx-auto text-purple-400 mb-1" />
          <div className="text-lg font-bold text-foreground">${paidInstallments.toFixed(0)} / ${totalInstallments.toFixed(0)}</div>
          <div className="text-[10px] text-muted-foreground/70">Installments</div>
        </Card>
        <Card className="luxury-card p-3 text-center">
          <Shield className="w-5 h-5 mx-auto text-emerald-400 mb-1" />
          <div className="text-lg font-bold text-foreground">${totalPremiums.toFixed(0)}</div>
          <div className="text-[10px] text-muted-foreground/70">Insurance ({insurance.length})</div>
        </Card>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList className="bg-muted/50 border border-border">
          <TabsTrigger value="maintenance" className="text-xs data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Maintenance</TabsTrigger>
          <TabsTrigger value="fuel" className="text-xs data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Fuel</TabsTrigger>
          <TabsTrigger value="installments" className="text-xs data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Installments</TabsTrigger>
          <TabsTrigger value="insurance" className="text-xs data-[state=active]:bg-muted data-[state=active]:text-foreground text-muted-foreground">Insurance</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="maintenance">
          ...
          {maintenance.length === 0 ? <p className="text-muted-foreground/50 text-sm text-center py-8">No maintenance records</p> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow className="border-border">
                  ...
                  <TableHead className="text-muted-foreground">Next</TableHead>
                  <TableHead />
                </TableRow></TableHeader>
                <TableBody>
                  {maintenance.map((m: any) => (
                    <TableRow key={m.id} className="border-border">
                      <TableCell className="text-xs text-foreground/80">{m.service_date}</TableCell>
                      <TableCell><Badge className="text-[10px] bg-muted text-foreground/80">{m.type.replace("_", " ")}</Badge></TableCell>
                      <TableCell className="text-xs text-muted-foreground max-w-[200px] truncate">{m.description || "—"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{m.mileage_at_service?.toLocaleString() || "—"}</TableCell>
                      <TableCell className="text-xs font-medium text-foreground">${Number(m.cost).toFixed(2)}</TableCell>
                      <TableCell className="text-xs text-muted-foreground/70">{m.next_service_mileage?.toLocaleString() || "—"}</TableCell>
        ...
        <TabsContent value="fuel">
          ...
          {fuel.length === 0 ? <p className="text-muted-foreground/50 text-sm text-center py-8">No fuel records</p> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow className="border-border">
                  <TableHead className="text-muted-foreground">Date</TableHead>
                  <TableHead className="text-muted-foreground">Gallons</TableHead>
                  <TableHead className="text-muted-foreground">Cost</TableHead>
                  <TableHead className="text-muted-foreground">Mileage</TableHead>
                ...
                <TableBody>
                  {fuel.map((f: any) => (
                    <TableRow key={f.id} className="border-border">
                      <TableCell className="text-xs text-foreground/80">{f.fuel_date}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{f.gallons ?? "—"}</TableCell>
                      <TableCell className="text-xs font-medium text-foreground">${Number(f.cost).toFixed(2)}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{f.mileage_at_fill?.toLocaleString() || "—"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground/70 max-w-[150px] truncate">{f.notes || "—"}</TableCell>
        ...
        <TabsContent value="installments">
          ...
          {installments.length === 0 ? <p className="text-muted-foreground/50 text-sm text-center py-8">No installments</p> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow className="border-border">
                ...
                <TableBody>
                  {installments.map((i: any) => (
                    <TableRow key={i.id} className="border-border">
                      <TableCell className="text-xs text-foreground/80">{i.due_date}</TableCell>
                      <TableCell className="text-xs font-medium text-foreground">${Number(i.amount).toFixed(2)}</TableCell>
                      <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => toggleInstallmentPaid(i)} className={i.paid ? "text-emerald-400" : "text-amber-400"}>
                          {i.paid ? "✓ Paid" : "Pending"}
                        </Button>
                      </TableCell>
                      <TableCell className="text-xs text-muted-foreground/70">{i.paid_date || "—"}</TableCell>
                      <TableCell className="text-xs text-muted-foreground/70 max-w-[150px] truncate">{i.notes || "—"}</TableCell>
        ...
        <TabsContent value="insurance">
          ...
          {insurance.length === 0 ? <p className="text-muted-foreground/50 text-sm text-center py-8">No insurance records</p> : (
            <div className="overflow-x-auto">
              <Table>
                <TableHeader><TableRow className="border-border">
                ...
                <TableBody>
                  {insurance.map((i: any) => (
                    <TableRow key={i.id} className="border-border">
                      <TableCell className="text-xs font-medium text-foreground">{i.provider}</TableCell>
                      <TableCell className="text-xs text-muted-foreground">{i.policy_number || "—"}</TableCell>
                      <TableCell className="text-xs font-medium text-foreground">${Number(i.premium).toFixed(2)}</TableCell>
                      <TableCell className="text-xs text-foreground/80">{i.start_date}</TableCell>
                      <TableCell className="text-xs text-foreground/80">{i.end_date || "—"}</TableCell>
      ...
      <Dialog open={mDialog} onOpenChange={setMDialog}>
        <DialogContent className="luxury-card border-border text-foreground max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div><Label className="text-xs text-muted-foreground">Description</Label><Input value={mForm.description} onChange={(e) => setMForm({ ...mForm, description: e.target.value })} className="input-luxury" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Service Date</Label><Input value={mForm.service_date} onChange={(e) => setMForm({ ...mForm, service_date: e.target.value })} className="input-luxury" type="date" /></div>
              <div><Label className="text-xs text-muted-foreground">Cost ($)</Label><Input value={mForm.cost} onChange={(e) => setMForm({ ...mForm, cost: e.target.value })} className="input-luxury" type="number" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Mileage at Service</Label><Input value={mForm.mileage_at_service} onChange={(e) => setMForm({ ...mForm, mileage_at_service: e.target.value })} className="input-luxury" type="number" /></div>
              <div><Label className="text-xs text-muted-foreground">Next Service Mileage</Label><Input value={mForm.next_service_mileage} onChange={(e) => setMForm({ ...mForm, next_service_mileage: e.target.value })} className="input-luxury" type="number" /></div>
            </div>
          </div>
      ...
      <Dialog open={fDialog} onOpenChange={setFDialog}>
        <DialogContent className="luxury-card border-border text-foreground max-w-md">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Fuel Date</Label><Input value={fForm.fuel_date} onChange={(e) => setFForm({ ...fForm, fuel_date: e.target.value })} className="input-luxury" type="date" /></div>
              <div><Label className="text-xs text-muted-foreground">Cost ($)</Label><Input value={fForm.cost} onChange={(e) => setFForm({ ...fForm, cost: e.target.value })} className="input-luxury" type="number" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Gallons</Label><Input value={fForm.gallons} onChange={(e) => setFForm({ ...fForm, gallons: e.target.value })} className="input-luxury" type="number" step="0.01" /></div>
              <div><Label className="text-xs text-muted-foreground">Mileage</Label><Input value={fForm.mileage_at_fill} onChange={(e) => setFForm({ ...fForm, mileage_at_fill: e.target.value })} className="input-luxury" type="number" /></div>
            </div>
            <div><Label className="text-xs text-muted-foreground">Notes</Label><Input value={fForm.notes} onChange={(e) => setFForm({ ...fForm, notes: e.target.value })} className="input-luxury" /></div>
          </div>
      ...
      <Dialog open={iDialog} onOpenChange={setIDialog}>
        <DialogContent className="luxury-card border-border text-foreground max-w-md">
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Amount ($)</Label><Input value={iForm.amount} onChange={(e) => setIForm({ ...iForm, amount: e.target.value })} className="input-luxury" type="number" /></div>
              <div><Label className="text-xs text-muted-foreground">Due Date</Label><Input value={iForm.due_date} onChange={(e) => setIForm({ ...iForm, due_date: e.target.value })} className="input-luxury" type="date" /></div>
            </div>
            <div><Label className="text-xs text-muted-foreground">Notes</Label><Input value={iForm.notes} onChange={(e) => setIForm({ ...iForm, notes: e.target.value })} className="input-luxury" /></div>
          </div>
      ...
      <Dialog open={insDialog} onOpenChange={setInsDialog}>
        <DialogContent className="luxury-card border-border text-foreground max-w-md">
          ...
          <div className="space-y-3">
            <div><Label className="text-xs text-muted-foreground">Provider *</Label><Input value={insForm.provider} onChange={(e) => setInsForm({ ...insForm, provider: e.target.value })} className="input-luxury" /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Policy #</Label><Input value={insForm.policy_number} onChange={(e) => setInsForm({ ...insForm, policy_number: e.target.value })} className="input-luxury" /></div>
              <div><Label className="text-xs text-muted-foreground">Premium ($)</Label><Input value={insForm.premium} onChange={(e) => setInsForm({ ...insForm, premium: e.target.value })} className="input-luxury" type="number" /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs text-muted-foreground">Start Date</Label><Input value={insForm.start_date} onChange={(e) => setInsForm({ ...insForm, start_date: e.target.value })} className="input-luxury" type="date" /></div>
              <div><Label className="text-xs text-muted-foreground">End Date</Label><Input value={insForm.end_date} onChange={(e) => setInsForm({ ...insForm, end_date: e.target.value })} className="input-luxury" type="date" /></div>
            </div>
            <div><Label className="text-xs text-muted-foreground">Notes</Label><Input value={insForm.notes} onChange={(e) => setInsForm({ ...insForm, notes: e.target.value })} className="input-luxury" /></div>
          </div>
